...
<div>
	...
	<table style="margin-right: 180px;	" class="table table-striped" id="contentTable">
	...
	<tbody>
		<?php
			$userList = $db->query("SELECT co.Id, u.FirstName, u.LastName, co.Topic, co.Text, cat.Name, co.CreationDate FROM contents co INNER JOIN users u ON co.Owner = u.Id INNER JOIN categories cat ON co.Category = cat.Id WHERE co.Confirm = 1 ORDER BY co.Id Desc");
		...
		<tr>
            ...
            <td><?php echo $Bol=substr(resimKaldir($contentText),0,70);?></td>
			<td><?php echo $contentCreatDate; ?></td>
			<td><?php echo $contentOwnerName." ".$contentOwnerSurname; ?></td>
			<td>
				<button class="btn btn-secondary btn-xs removeContentConfirm" id="<?php echo $contentId; ?>">Yayından Kaldır</button> | 
    ...
    <div class="modal-content">
        <form id="fupForm" enctype="multipart/form-data">
	      ...
	      <div class="modal-body">
              ...
              <div class="form-group">
                  <label>Başlık</label>
                  <input class="form-control" type="text" name="newTopic" id="newTopic" placeholder="Başlık Giriniz" required>
              </div>
...
$(document).ready(function(e){
    ...
    $("#fupForm").on('submit', function(e){
        ...
        $.ajax({
            ...
            cache: false,
            processData:false,
           success: function(response){ console.log(this.data);
                $('.statusMsg').html('');
	...
	$(document).on('click','.removeContentConfirm',function(){


		var edit_id = $(this).attr('id');

		$.ajax({
					url:"removeContentConfirm.php",
					type:"post",
					data:{edit_id:edit_id},
					success:function(data){
						alert("İçerik onayı kaldırıldı ve yayından alındı.");
						location.reload();
					}
				});
